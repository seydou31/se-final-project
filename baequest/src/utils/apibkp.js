...
// ── Event Manager: Dashboard Stats (no events) ──────────────────
async function eventManagerGetDashboardStats() {
  const res = await fetch(`${baseUrl}/event-managers/dashboard/stats`, {
    credentials: 'include',
  });
...
// ── Event Manager: Paginated Events List ────────────────────────
async function eventManagerGetEvents({ page = 1, limit = 10, search = '' } = {}) {
  const params = new URLSearchParams({ page, limit });
  if (search) params.set('search', search);
  const res = await fetch(`${baseUrl}/event-managers/events?${params}`, {
    credentials: 'include',
  });
  if (!res.ok) throw new Error('Failed to load events');
  return res.json();
  // Expected shape: { data: [...], pagination: { page, limit, total, totalPages } }
}

async function eventManagerLogout() {
  const res = await fetch(`${baseUrl}/event-managers/logout`, {
    method: 'POST',
    credentials: 'include',
  });
